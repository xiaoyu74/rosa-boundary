#!/usr/bin/env bash
#
# platform_convert — translate @@PLATFORM@@ placeholders to architecture-specific strings.
#
# Used in multi-arch Containerfile builds to resolve download URLs at build time.
# When podman builds with --platform linux/arm64, uname -m returns "aarch64".
# For --platform linux/amd64, uname -m returns "x86_64".
#
# Usage:
#   platform_convert "url-with-@@PLATFORM@@" --arm64 --amd64
#   platform_convert "url-with-@@PLATFORM@@" --aarch64 --x86_64
#   platform_convert "url-with-@@PLATFORM@@" --custom-arm64 custom_string --custom-amd64 custom_string
#   platform_convert --file path/to/file --arm64 --amd64
#
...
if [[ -n $REPLACESTR ]] && [[ $(grep --count "@@PLATFORM@@" <<< "$REPLACESTR") -eq 0 ]]
then
